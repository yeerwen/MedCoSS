...
def get_arguments():
    ...
    parser.add_argument("--model_name", type=str, default='teacher or student or model')
...
def main():
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        shutil.copytree("Downstream/Dim_1/PudMed20k", os.path.join(args.snapshot_dir, "code"))
        ...
        trainloader, train_sampler = engine.get_train_loader(
            PudMed_20k_Dataset(args.data_path, split="train", max_words=args.input_size),
            drop_last=True)
        valloader, val_sampler = engine.get_test_loader(
            PudMed_20k_Dataset(args.data_path, split="dev", max_words=args.input_size), batch_size=1)

        testloader, test_sampler = engine.get_test_loader(
            PudMed_20k_Dataset(args.data_path, split="test", max_words=args.input_size), batch_size=1)
        ...
        for epoch in range(start_epoch, args.num_epochs):
            ...
            for iter, (input_ids, labels, att_masks) in tqdm(enumerate(trainloader)):
                ...
                if args.local_rank == 0 and (iter + 1) % 500 == 0:
                    ...
                    label_val = []
                    with torch.no_grad():
                        for iter, (input_ids, labels, att_masks) in tqdm(enumerate(valloader)):
                            input_ids = input_ids.cuda(non_blocking=True)
                            labels = labels.squeeze(1).long().cuda(non_blocking=True)
                            att_masks = att_masks.cuda(non_blocking=True)
                            data = {"data": input_ids, "text_labels": labels,
