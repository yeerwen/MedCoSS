...
from DSDataset import Glas_Dataset, collate_fn_ts
import random
import timeit
...
def multi_net(net_list, img):
    ...
    for i in range(1, len(net_list)):
        padded_prediction_i = net_list[i](data)
...
def predict_sliding(args, net_list, image, tile_size, classes, gaussian_importance_map):  # tile_size:32x256x256
    ...
    for row in range(tile_rows):
        for col in range(tile_cols):
            ...
            prediction4 = torch.flip(multi_net(net_list, torch.flip(img, [2, 3])), [2, 3])
...
def save_nii(args, pred, name):  # bs, c, WHD
    ...
    cv2.imwrite(os.path.join(args.save_path, name[0]+".png"), segmentation)
    return None
...
def validate(args, input_size, model, ValLoader, num_classes):
    ...
    for root, dirs, files in os.walk(args.save_path):
        for i in  tqdm(sorted(files)):
            ...
            i2_file = os.path.join(os.environ["nnUNet_preprocessed"], 'test', "labels", i)
...
def main():
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        valloader, val_sampler = engine.get_test_loader(
           Glas_Dataset(args.data_dir, args.val_list, split="test"), collate_fn=collate_fn_ts)
