...
def main():
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        shutil.copytree("Downstream/Dim_2/NCT_CRC_HE", os.path.join(args.snapshot_dir, "code"))
        shutil.copyfile("run_ds.sh", os.path.join(args.snapshot_dir, "code", "run_ds.sh"))
        print("code copy!")

        if args.arch == "unified_vit":
            model = Unified_Model(num_classes=args.num_classes, pre_trained=args.reload_from_pretrained, pre_trained_weight=args.pretrained_path)
            print("unified_vit")
        ...
        trainloader, train_sampler = engine.get_train_loader(
            NCT_CRC_HE_Dataset(args.data_path, split="train", crop_size=input_size),
            drop_last=True)

        valloader, val_sampler = engine.get_test_loader(
            NCT_CRC_HE_Dataset(args.data_path, split="val", crop_size=input_size), batch_size=1)

        testloader, test_sampler = engine.get_test_loader(
            NCT_CRC_HE_Dataset(args.data_path, split="test", crop_size=input_size), batch_size=1)
        ...
        for epoch in range(start_epoch, args.num_epochs):
            ...
            for iter, (input_ids, labels) in tqdm(enumerate(trainloader)):
                ...
                if args.local_rank == 0 and (iter + 1) % 1000 == 0:
                    model.eval()
                    model.cal_acc = True
                    pre_score = []
                    label_val = []
