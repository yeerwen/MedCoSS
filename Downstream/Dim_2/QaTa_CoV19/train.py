...
from DSDataset import QaTa_CoV19_Dataset, collate_fn_tr, collate_fn_ts
import random
import timeit, time
...
def get_arguments():
    ...
    parser.add_argument("--ratio_labels", type=float, default=1.0)
...
def main():
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        # print(os.getcwd())
        shutil.copytree("Downstream/Dim_2/QaTa_CoV19", os.path.join(args.snapshot_dir, "code"))
        shutil.copyfile("run_ds.sh", os.path.join(args.snapshot_dir, "code", "run_ds.sh"))
        print("code copy!")

        if args.arch == "unified_vit":
            model = Unified_Model(now_2D_input_size=input_size, in_chans=3, num_classes=args.num_classes, pre_trained=args.reload_from_pretrained, pre_trained_weight=args.pretrained_path, model_name=args.model_name)
            print("unified_vit")
        ...
        model.to(device)
        print("no grad params:")
        for p, q in model.named_parameters():
            if q.requires_grad == False:
                print(p, end=" ")
        print()
        # print(model)
        ...
        trainloader, train_sampler = engine.get_train_loader(
            QaTa_CoV19_Dataset(args.data_dir, args.train_list, max_iters=args.itrs_each_epoch * args.batch_size,
                        crop_size=input_size, scale=args.random_scale, mirror=args.random_mirror, split="train", ratio_labels=args.ratio_labels),
            drop_last=True, collate_fn=collate_fn_tr)

        valloader, val_sampler = engine.get_test_loader(
            QaTa_CoV19_Dataset(args.data_dir, args.val_list,
