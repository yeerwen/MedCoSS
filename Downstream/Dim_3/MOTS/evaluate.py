...
from dataloader import MOTS_Dataset_test
import timeit
...
def validate(args, input_size, model, ValLoader, num_classes, engine, json_dict):
    ...
    val_Dice = torch.zeros(size=(1, 2))
    val_HD = torch.zeros(size=(1, 2))
    count_Dice = torch.zeros(size=(1, 2))
    count_HD = torch.zeros(size=(1, 2))
    ...
    for root, dirs, files in os.walk(args.save_path):
        for i in sorted(files):
            ...
            if task_id==0:
                ...
                val_HD[task_id, 1] += HD_2
                count_Dice[task_id, 0] += 1
                count_Dice[task_id, 1] += 1
                count_HD[task_id, 0] += 1
                count_HD[task_id, 1] += 1
            log_i = ("Organ-[Dice-%.4f; HD-%.4f], tumor-[Dice-%.4f; HD-%.4f]" % (dice_1, HD_1, dice_2, HD_2))
            print("%s: %s" % (i,log_i))
            json_dict[i]=log_i
    ...
    print("Sum results")
    for t in range(1):
        print('Sum: Task%d- Organ:[Dice-%.4f; HD-%.4f] Tumor:[Dice-%.4f; HD-%.4f]' %
              (t, val_Dice[t, 0], val_HD[t, 0], val_Dice[t, 1], val_HD[t, 1]))
...
def main():
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        valloader, val_sampler = engine.get_test_loader(MOTS_Dataset_test(args.data_dir, args.val_list), 1)
