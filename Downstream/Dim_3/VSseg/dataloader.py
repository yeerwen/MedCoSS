...
class VSseg_Dataset_train(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(64, 192, 192), mean=(128, 128, 128), scale=True,
        ...
        for item in self.img_ids:
            ...
            img_gt_file = osp.join(preprocessed_data_path, 'nnUNetData_plans_v2.1_stage1', img_identifier + '.npz')
    ...
    def locate_bbx(self, label, class_locs):
        ...
        if random.random() < 0.5:
            selected_class = np.random.choice(list(class_locs.keys()), 1)
            # print(selected_class)
            if len(class_locs[selected_class[0]]) == 0:
                ...
                selected_voxel = class_locs[selected_class[0]][np.random.choice(len(class_locs[selected_class[0]]))]
...
class VSseg_Dataset_test(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(64, 192, 192), mean=(128, 128, 128), scale=True,
                 mirror=True, ignore_label=255, ratio_labels=1):
