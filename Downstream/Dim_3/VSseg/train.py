...
# from DS.models.nnFormer_Unet import nnFormer_Unet
from dataloader import VSseg_Dataset_train, my_collate
...
def main():
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        # print(os.getcwd())
        shutil.copytree("Downstream/Dim_3/VSseg", os.path.join(args.snapshot_dir, "code"))
        ...
        trainloader, train_sampler = engine.get_train_loader(
            VSseg_Dataset_train(args.data_dir, args.train_list, max_iters=args.itrs_each_epoch * args.batch_size,
        ...
        for epoch in range(start_epoch, args.num_epochs):
            ...
            for iter, batch in enumerate(trainloader):
                ...
                # print(images.size(), labels.size(), torch.min(images), torch.max(images), torch.min(labels), torch.max(labels))
                data = {"data": images, "modality": "3D image"}
                optimizer.zero_grad()
