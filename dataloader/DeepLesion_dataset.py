...
class DeepLesion_dataset(Dataset):
    def __init__(self, data_path, crop_size=(16, 192, 192), is_sort=False):
        ...
        if not is_sort:
            self.img_ids = [i_id.strip().split() for i_id in open(os.path.join(data_path, "SSL_data_deeplesion.txt"))]
        else:
            self.img_ids = sorted([i_id.strip().split() for i_id in open(os.path.join(data_path, "SSL_data_deeplesion.txt"))])
        ...
        for nii_name in self.img_ids:
            ...
            self.files.append({
                "img": img_file.replace("DL_patches_v2", "DL_patches_v2_resize"),
                "name": nii_name[0]
            })
...
class DeepLesion_dataset_name(Dataset):
    def __init__(self, data_path, crop_size=(16, 192, 192), is_sort=False):
