...
from dataloader.DeepLesion_dataset import DeepLesion_dataset
from dataloader.ADNI_dataset import ADNI_dataset
from dataloader.TCGA_dataset import TCGA_Image_Dataset
...
def get_args_parser():
    ...
    #task 1D 2D 3D
    parser.add_argument('--task_modality', default="1D_text", choices=["1D_text", "2D_xray", "3D_CT", "3D_MR", "2D_path", "1D_2D_vl"], type=str,
...
def main(args):
    ...
    if args.task_modality == "1D_text":
        dataset_train = MIMIC_CXR_Report_Dataset(data_path=args.data_path, split="train", transform=None, max_words=112)
    elif args.task_modality == "2D_xray":
        dataset_train =  MIMIC_CXR_Image_Dataset(data_path=args.data_path, imsize=args.input_size)
    elif args.task_modality == "3D_CT":
        dataset_train = DeepLesion_dataset(data_path=args.data_path, crop_size=(16, 192, 192))
    elif args.task_modality == "3D_MR":
        dataset_train = ADNI_dataset(data_path=args.data_path, crop_size=(16, 192, 192))
    elif args.task_modality == "2D_path":
        dataset_train =  TCGA_Image_Dataset(data_path=args.data_path, imsize=args.input_size)
    else:
        exit()
    ...
    pre_dict_no_update = [k for k in pre_dict.keys() if (k not in model_dict or sum([module not in k for module in updata_module]) == len(updata_module)) ]
...
if __name__ == '__main__':
    ...
    if misc.is_main_process():
        ...
        shutil.copyfile("engine_pretrain.py", os.path.join(args.output_dir, "code", "engine_pretrain.py"))
        shutil.copyfile("main_pretrain_single_modal.py", os.path.join(args.output_dir, "code", "main_pretrain_single_modal.py"))
    main(args)
